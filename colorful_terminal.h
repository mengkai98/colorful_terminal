...
{   
   ...
   #define CTL_AUTOTP(TAG) ctl::hp_out()<<"id: "<<ctl::record_points_num++<<" ["<<__FILE__<<"] : "<<"("<<__LINE__<<")"<<" tag: "<<TAG<<ctl::endl
   #define CTL_TITLE_EXIT(INFO) ctl::hr_out()<<" ["<<__FILE__<<"] : "<<"("<<__LINE__<<")"<<" exit: "<<INFO<<ctl::endl;exit(0)
    enum COLOR {
        BLACK = 0,
        RED,
        GREEN,
    ...
    std::string generate_ansi(enum COLOR front_color =CRUNUSED,enum CTRL ctrl = CLUNUSED,enum COLOR back_color = CRUNUSED){
    ...
    std::ostream & out(enum COLOR front_color =CRUNUSED,enum CTRL ctrl = CLUNUSED,enum COLOR back_color = CRUNUSED){
        std::string generate_ansi;
        if (ctrl != CLUNUSED)
        {
            generate_ansi += std::to_string(ctrl);
    ...
    public:
        ...
        void make_table_and_out(){
            ...
            table_txt+=generate_ansi(line_color.front_color,line_color.ctl,line_color.back_color);
            table_txt+=tline;
            table_txt+=ctl::endl;

            std::string vline = generate_ansi(line_color.front_color,line_color.ctl,line_color.back_color)+"|\033[0m";

            std::string tmp;
            tmp=vline+padding_txt+generate_ansi(title_color.front_color,title_color.ctl,title_color.back_color)+table_name+"\033[0m";
            ...
            for (size_t i = 0; i < var_infos.size(); i++)
            {
                ...
                auto &vi =var_infos[i];
                tmp.clear();
                tmp=vline+padding_txt+generate_ansi(now_scheme.front_color,now_scheme.ctl,now_scheme.back_color)+vi.var_name+"\033[0m";
                tmp+=std::string(width[0]-vi.var_name.size(),' ');
                tmp+=padding_txt+vline+padding_txt+generate_ansi(now_scheme.front_color,now_scheme.ctl,now_scheme.back_color)+vi.var_type+"\033[0m";
                tmp+=std::string(width[1]-vi.var_type.size(),' ');
                tmp+=padding_txt+vline+padding_txt+generate_ansi(now_scheme.front_color,now_scheme.ctl,now_scheme.back_color)+vi.var_val+"\033[0m";
